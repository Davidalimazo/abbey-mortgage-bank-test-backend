...
import { PromotionSearchCriteria } from "./interface";
...
class PromotionRepository{
  ...
  try {
    const truncateSQL = `TRUNCATE TABLE PROMOTION`;
    await connection.execute(truncateSQL);

    let rows;

    const sql = `INSERT INTO PROMOTION (
      PERSONNEL_NO,
      RANK,
      DATE_OF_LAST_PROMOTION,
      SENIORITY_ON_EACH_RANK,
       AUTHORITY_FOR_EACH_RANK
      ) VALUES ( 
        :PERSONNEL_NO,
        :RANK,
        :DATE_OF_LAST_PROMOTION,
        ...
        return {
          PERSONNEL_NO:item["PERSONNEL NO"],
                    ...
                    SENIORITY_ON_EACH_RANK: item["SENIORITY ON EACH RANK"] ? new Date(item["SENIORITY ON EACH RANK"]) : null,
                     AUTHORITY_FOR_EACH_RANK: item["PROMOTION AUTHORITY FOR EACH RANK"]
        }
      });
        ...
        const options = {
          ...
          bindDefs: {
            ...
            SENIORITY_ON_EACH_RANK: { type: Oracledb.DATE},
             AUTHORITY_FOR_EACH_RANK: { type: Oracledb.STRING, maxSize:50 },
          }
        };
   ...
   return error.message;
    ...
    async findPromotionHistoryByPersonalNo(personalNo:string){
        ...
        try {
            ...
            const sql = `SELECT * FROM PROMOTION WHERE PERSONNEL_NO = ${personalNo}`;
...
async findPromotionHistoryByCriteria(criteria:PromotionSearchCriteria, data:string){
  let connection =  await connectToDB();
  

  
  try {
      const sql = `SELECT * FROM PROMOTION WHERE ${PromotionSearchCriteria[criteria]} = :1`;
...
async findFirstPromotionDetails(id:string){
  ...
  try {
      const sql = `SELECT *
      FROM PROMOTION where personnel_no='${id}'
